...
export async function POST(req: NextRequest) {
  ...
  const { staff_id, event_id, admin_password, admin_phone, notes } = body ?? {};
  const errors: Record<string, string[]> = {};

  if (!staff_id) errors.staff_id = ["staff_id is required."];
  ...
  const upstream = await fetch(
    `${DJANGO_API_URL}/api/attendance/admin-clock/manual-check-out/`,
    {
      method: "POST",
