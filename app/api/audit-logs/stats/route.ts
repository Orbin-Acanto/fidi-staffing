...
const ALLOWED_PARAMS = ["days", "company"];
...
export async function GET(req: NextRequest) {
  ...
  const url = `${DJANGO_API_URL}/api/audit-logs/stats/${queryString ? `?${queryString}` : ""}`;
  ...
  if (!upstream.ok) {
    return NextResponse.json(
      {
        message:
          data?.message || data?.detail || "Failed to fetch audit log stats.",
