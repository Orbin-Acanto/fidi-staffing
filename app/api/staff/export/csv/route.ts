...
export async function GET(req: NextRequest) {
  ...
  const url = `${DJANGO_API_URL}/api/staff/export/csv/${queryString ? `?${queryString}` : ""}`;
  ...
  if (!upstream.ok) {
    ...
    return NextResponse.json(
      {
        message: data?.message || data?.detail || "Failed to export staff CSV.",
  ...
  const csvData = await upstream.blob();
  const filename =
    upstream.headers
      .get("Content-Disposition")
      ?.split("filename=")[1]
      ?.replace(/"/g, "") || "staff_export.csv";
