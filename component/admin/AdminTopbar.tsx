    ...
    async function fetchCompanies() {
      try {
        ...
        if (me?.current_company?.id) {
          setCurrentCompanyId(me.current_company.id);
        } else if (response.companies?.[0]) {
          setCurrentCompanyId(response.companies[0].id);
        }
      } catch (error) {
      ...
      <div className="flex items-center gap-4 ml-4">
        <div className="relative">
          <AppSelect
            ...
            disabled={
              isLoadingCompanies || isSwitching || companies.length === 0
            }
            placeholder={
